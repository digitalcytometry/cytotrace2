  ...
  suppressMessages({
    suppressWarnings({
      ...
      potency_score_umap <- FeaturePlot(seurat, "CytoTRACE2_Score_clipped") +
        scale_colour_gradientn(colours = rev(colors), na.value = "transparent", limits = c(-5, 0), name = "Potency score \n") +
      ...
      if (!is.null(annotation)) {
        ...
        phenotype_umap <- DimPlot(seurat, reduction = "umap", group.by = "Phenotype_CT2", label = FALSE) +
        ...
        potencyBoxplot_byPheno <- ggplot(mtd[!is.na(mtd$Phenotype_CT2), ], aes(x = Phenotype_CT2, y = CytoTRACE2_Score)) +
          ...
          theme(legend.position = "None", axis.text = element_text(size = 8), axis.text.x = element_text(angle = 90, hjust = 0.5),
