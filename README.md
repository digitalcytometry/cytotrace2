...
<br>
 ...
 - <details> <summary> <strong>Other output plots</strong> </summary>
    ...
    <br> A boxplot of predicted potency score separated by phenotype/group from annotation file. Can be used to assess the distribution of predicted potency scores across different cell phenotypes. <br>
      ```bash
...
- To pass a Seurat object or .rds file as gene expression input, users should specify ```is_seurat = TRUE```
...
To illustrate use of CytoTRACE 2 with a human dataset stored in a Seurat object, we will use the Seurat object Cord_blood_CITE_seq_SeuratObj.rds, originally from [Stoeckius et al., 2017](https://doi.org/10.1038/nmeth.4380), filtered to cells with known ground truth developmental potential and available to download [here](https://drive.google.com/uc?export=download&id=1DzG1qR_xUSt-HzLKSY19vO9b6EugbQvG), containing:
- gene expression data for a scRNA-seq (CITE-seq) dataset comprising of 8617 cord blood mononuclear cells. Stored inn the RNA assay of the Seurat object.
- phenotype annotations for the scRNA-seq dataset above. Stored in the metadata of the Seurat object.

The dataset contains only mononuclear cells, hence we would expect to see CytoTRACE 2 predictions for potency scores with the majority, if not all of cells falling in the differentiated category.
...
```r
# download the .rds file (this will download the file to your working directory)
download.file("https://drive.google.com/uc?export=download&id=1DzG1qR_xUSt-HzLKSY19vO9b6EugbQvG", "Cord_blood_CITE_seq_downsampled_SeuratObj.rds")# load the Seurat object
...
- ***Potency score distribution by phenotype***
<br> A boxplot of predicted potency score separated by phenotype/group from annotation file. Can be used to assess the distribution of predicted potency scores across different cell phenotypes.
  ```bash
  plots$CytoTRACE2_Boxplot_byPheno
  ```
  <p align="center">
...
Please note that all instances of duplicate gene names will be dropped at runtime.

<p align="center">
    <img width="600" src="images/data.png">
</p>

CytoTRACE 2 also accepts cell phenotype annotations as an optional second input. This is a non-required argument used for plotting outputs, and if provided, will be used to generate UMAP and predicted potency score box plots by phenotype. It should be passed as a loaded dataframe containing cell IDs in the first column and phenotype labels (string formatted without special characters) in the second column. Any additional columns will be ignored.
...
- **Phenotype potency box plot**: a boxplot of predicted potency score separated by phenotype/group from annotation file (named *CytoTRACE2_Boxplot_byPheno* in the output list)
...
```r
result <- cytotrace2("path/to/input/expression_file.tsv",
                       species = "mouse",
                       is_seurat = FALSE,
                       full_model = TRUE,
                       batch_size = 10000,
...
```r
expression_data <- loadData("path/to/input/expression_file.tsv")
cytotrace2_result <- cytotrace2(exoression_data,   
                  species = "human",
                  is_seurat = TRUE,
                  slot_type = "counts",
                  full_model = TRUE,
...
#' @param data Dataframe, expression data to be used for plotting
...
Required input:
- *cytotrace2*: Output of the cytotrace2 function as described in [_CytoTRACE 2 outputs__](#cytotrace-2-outputs)

Optional input:
- *data*: The expression data to be used for plotting (default is ***NULL***, if cytotrace2 is a Seurat object containing expression data and is_seurat = TRUE, can be left NULL).
...
1. **Do the R and Python packages produce equivalent output?**
When run without batching, these packages produce equivalent output. Due to the inherent randomness, package outputs will be different when batching is included, but the general performance should remain similar.
...
5. **What are the CytoTRACE 2 potency categories?**
